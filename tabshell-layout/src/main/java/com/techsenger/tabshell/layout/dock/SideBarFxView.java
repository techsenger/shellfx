...
public class SideBarFxView<P extends SideBarPresenter<?, ?>> extends AbstractAreaFxView<P> implements SideBarView {
    ...
    public class Composer extends AbstractAreaFxView.Composer {
        ...
        public DockLayoutPort getLayout() {
            return view.getLayout().getPresenter().getPort();
        }
        ...
        void addPopupToLayout(Side side) {
            ...
            p.initialize();
            view.getLayout().getComposer().addTabPopup(v);
        }

        void removePopupFromLayout() {
            if (getPopup() != null) {
                view.layout.getComposer().removeTabPopup(view.getPresenter().getSide());
            }
        }
    }
    ...
    public SideBarFxView(DockLayoutFxView<?> layout) {
        this.layout = layout;
    }
    ...
    protected void onRestoreButtonAction(Tab tab) {
        ...
        if (tabDocks.isEmpty() && layout.getComposer().getBarPolicy(side) != SideBarPolicy.EXISTS_ALWAYS) {
            layout.getComposer().removeBar(side);
        }
        layout.restoreTabDock(tabDock);
    }
    ...
    Dimension2D getCenterDimension() {
        return layout.getCenterDimension();
    }
    ...
    private TabPopupFxView<?> getPopup() {
        return layout.getPopup(getPresenter().getSide());
    }
