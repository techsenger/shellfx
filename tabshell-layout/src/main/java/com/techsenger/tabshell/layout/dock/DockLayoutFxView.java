 ...
 */
public class DockLayoutFxView<P extends DockLayoutPresenter<?, ?>> extends AbstractAreaFxView<P>
        implements DockLayoutView {

    private static final Logger logger = LoggerFactory.getLogger(DockLayoutFxView.class);
    ...
    private abstract static class AbstractContainer<T extends AreaFxView<?>> extends StackPane {
        ...
        AbstractContainer(DockLayoutFxView<?> layout, T area) {
            this.layout = layout;
            this.area = area;
            getChildren().add(area.getNode());
        }

        DockLayoutFxView<?> getLayout() {
            return layout;
        }
    ...
    private static class SplitSpaceContainer extends AbstractContainer<SplitSpaceFxView<?>> {
        ...
        SplitSpaceContainer(DockLayoutFxView<?> layout, SplitSpaceFxView<?> view) {
            super(layout, view);
    ...
    private abstract static class AbstractEventContainer<T extends AreaFxView<?>>
        ...
        AbstractEventContainer(DockLayoutFxView<?> layout, T area) {
            super(layout, area);
    ...
    private static class MainContainer extends AbstractEventContainer<AreaFxView<?>> {
        ...
        MainContainer(DockLayoutFxView<?> layout, AreaFxView<?> area) {
            super(layout, area);
        }
    ...
    private static class TabDockContainer extends AbstractEventContainer<TabDockFxView<?>> {
        ...
        TabDockContainer(DockLayoutFxView<?> layout, TabDockFxView<?> tabDock) {
    ...
    public class Composer extends AbstractAreaFxView.Composer implements DockLayoutComposer {
        ...
        public SplitSpacePort getRoot() {
            return view.getRoot() == null ? null : view.getRoot().getPresenter().getPort();
        }

        @Override
        public AreaPort getMain() {
            return view.getMain() == null ? null : view.getMain().getPresenter().getPort();
        }
        ...
        public SideBarPort getRightBar() {
            return view.getRightBar() == null ? null : view.getRightBar().getPresenter().getPort();
        }

        @Override
        public SideBarPort getBottomBar() {
            return view.getBottomBar() == null ? null : view.getBottomBar().getPresenter().getPort();
        }

        @Override
        public SideBarPort getLeftBar() {
            return view.getLeftBar() == null ? null : view.getLeftBar().getPresenter().getPort();
        }
        ...
        public SideBarPort getBar(Side side) {
            ...
            if (barFxView != null) {
                return barFxView.getPresenter().getPort();
            } else {
                return null;
            }
        }
        ...
        public TabPopupPort getRightPopup() {
            return view.getRightPopup() == null ? null : view.getRightPopup().getPresenter().getPort();
        }

        @Override
        public TabPopupPort getBottomPopup() {
            return view.getBottomPopup() == null ? null : view.getBottomPopup().getPresenter().getPort();
        }

        @Override
        public TabPopupPort getLeftPopup() {
            return view.getLeftPopup() == null ? null : view.getLeftPopup().getPresenter().getPort();
        }
    ...
    /**
     * Contains the {@link DockLayoutFxView#centerStackPane} and three {@link SideBarView}.
     */
    ...
    protected Composer createComposer() {
        return new DockLayoutFxView.Composer();
    }
    ...
    protected void build() {
        ...
        var css = DockLayoutFxView.class.getResource("dock-layout.css").toExternalForm();
    ...
    void minimizeTabDock(TabDockFxView<?> dock) {
        ...
        pos.buildMaps();
        dock.getPresenter().getPort().setMinimizedPosition(pos);
    ...
    void restoreTabDock(TabDockFxView<?> dock) {
        ...
        dock.getPresenter().getPort().setMinimizedPosition(null);
    ...
    private boolean checkNewSide(SplitSpaceFxView<?> parent, int index, Side side) {
        ...
        if (child == getMain()) {
            ...
            if (parentSplitSpace.getOrientation() == Orientation.HORIZONTAL) {
                resolvedSide = LEFT;
                if (isLastPosition) {
                    resolvedSide = RIGHT;
                }
    ...
    private SplitSpaceFxView<?> wrap(AreaFxView<?> area, int index) {
        ...
        var newSplitSpace = getComposer().createSplitSpace(newOrientation);
