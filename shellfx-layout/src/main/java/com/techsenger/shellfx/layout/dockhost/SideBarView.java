 ...
 */
public class SideBarView<VM extends SideBarViewModel<?>> extends AbstractAreaView<VM> {
    ...
    private static final class BarTab extends Tab {
        ...
        public void deinit() {
            TabView<?> tabFxView = (TabView<?>) ViewUtils.getView(this.minimizedTab);
            tabFxView.getViewModel().setClosable(this.minimizedTabClosable);
                ...
                if (isReady()) {
                    ...
                    if (!barTab.isShownInPopup()) {
                        if (getComposer().getPopup() == null) {
                            getComposer().addPopupToLayout(getViewModel().getSide());
                        }
    ...
    public class Composer extends AbstractAreaView<VM>.Composer implements SideBarComposer {

        private final SideBarView<VM> view = SideBarView.this;

        private DockHostView<?> dockHost;

        private final ObservableList<TabDockView<?>> modifiableTabDocks = FXCollections.observableArrayList();

        private final @Unmodifiable ObservableList<TabDockView<?>> tabDocks =
        ...
        public @Unmodifiable List<TabDockPort> getTabDockPorts() {
            return getTabDocks().stream()
                    .map(t -> (TabDockPort) t.getViewModel())
                    .toList();
        }
        ...
        public DockHostPort getLayoutPort() {
            return this.dockHost == null ? null : this.dockHost.getViewModel();
        }
        ...
        public @Unmodifiable ObservableList<TabDockView<?>> getTabDocks() {
            return tabDocks;
        }

        private TabPopupView<?> getPopup() {
            return dockHost.getComposer().getPopup(getViewModel().getSide());
        }

        void addTabDock(TabDockView<?> tabDock) {
            modifiableTabDocks.add(tabDock);
            getModifiableChildren().add(tabDock);
        }

        void removeTabDock(TabDockView<?> tabDock) {
            modifiableTabDocks.remove(tabDock);
            getModifiableChildren().remove(tabDock);
        }

        TabDockView<?> removeTabDock(int index) {
            TabDockView<?> tabDock = modifiableTabDocks.remove(index);
            getModifiableChildren().remove(tabDock);
            return tabDock;
        }
        ...
        void removePopupFromLayout() {
            if (getPopupPort() != null) {
                dockHost.getComposer().closeTabPopup(view.getViewModel().getSide());
            }
        }
    }
    ...
    public SideBarView(VM viewModel, DockHostView<?> dockHost) {
        super(viewModel);
    ...
    protected void addListeners() {
        super.addListeners();
        getComposer().tabDocks.addListener((ListChangeListener<TabDockView<?>>) (change) -> {
            while (change.next()) {
                if (change.wasAdded()) {
    ...
    protected void openTabInPopup(BarTab barTab) {
        ...
        TabView<?> tabFxView = (TabView<?>) ViewUtils.getView(minimizedTab);
        tabFxView.getViewModel().setClosable(false);
