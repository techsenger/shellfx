 ...
 * {@link AbstractAreaView} components into a hierarchy of SplitPanes, allowing users to freely reorganize the
 ...
 */
public class DockHostView<VM extends DockHostViewModel<?>> extends AbstractAreaView<VM> {
    ...
    protected static final class DragAndDropHandler {
        ...
        private final DockHostView<?> dockHost;

        private final DockHostView.Composer composer;
        ...
        public DragAndDropHandler(DockHostView<?> dockHost) {
         ...
         * @param position
         */
        void onDockDragDetected(TabDockView<?> dock, FontIconView iconView, MouseEvent e) {
            this.dragDock = dock;
        ...
        void onDockMouseDragged(TabDockView<?> dock, FontIconView iconView, MouseEvent e) {
        ...
        void onDockMouseReleased(TabDockView<?> dock, FontIconView iconView, MouseEvent e) {
            hideDragDockPopup();
        ...
        private void processDropInsideTabHeaderArea() {
            ...
            // is handled by TabPanePro and TabHostView handlers.
            ...
            if (dropPos != null && dropPos.getTransformation() != null && dropPos.isValid()) {
                ...
                if (this.dragDock == null) {
                    ...
                    logger.debug("{} Replaced {} with {}", dockHost.getDescriptor().getLogPrefix(),
                            composer.placeholder.getViewModel().getDescriptor().getFullName(),
                            dragDock.getViewModel().getDescriptor().getFullName());
                }
    ...
    private abstract static class AbstractContainer extends StackPane {
        ...
        AbstractContainer(DockHostView<?> dockHost) {
            this.dockHost = dockHost;
        }

        DockHostView<?> getDockHost() {
            return dockHost;
        }
    ...
    private static final class SplitPaneContainer extends AbstractContainer {
        ...
        SplitPaneContainer(DockHostView<?> dockHost, DockSplitPane splitPane) {
            super(dockHost);
     ...
     * removed from there.
     */
    private abstract static class AbstractAreaContainer<T extends AreaView<?>> extends AbstractContainer {
        ...
        AbstractAreaContainer(DockHostView<?> dockHost, T area) {
            super(dockHost);
            this.area = area;
        ...
        @Override
        String getChildName() {
            return area.getViewModel().getDescriptor().getName().getText();
        }

        @Override
        String getChildFullName() {
            return area.getViewModel().getDescriptor().getFullName();
        }

        @Override
        UUID getChildUuid() {
            return area.getViewModel().getDescriptor().getUuid();
        }

        @Override
        String getChildShortUuid() {
            return area.getViewModel().getDescriptor().getShortUuid();
        }
    ...
    private static final class TabDockContainer extends AbstractAreaContainer<TabDockView<?>> {

        TabDockContainer(DockHostView<?> dockHost, TabDockView<?> tabDock) {
    ...
    private static final class ContainerUtils {
        ...
        private static SplitPaneContainer createContainer(DockHostView<?> dockHost, DockSplitPane splitPane) {
            var container = new SplitPaneContainer(dockHost, splitPane);
            SplitPane.setResizableWithParent(container, false);
            return container;
        }

        private static AbstractAreaContainer<?> createContainer(DockHostView<?> dockHost, AreaView<?> child) {
            AbstractAreaContainer<?> container;
            if (child instanceof TabDockView<?>) {
         ...
         * @return
         */
        private static AbstractAreaContainer<?> getContainer(AreaView<?> view) {
            return (AbstractAreaContainer<?>) view.getNode().getParent();
        }

        private static boolean hasContainer(AreaView<?> view) {
         ...
         * <p>
         * Only nodes obtained from {@link Composer#getModelNode(AreaView)} — or reached from it by navigating via
        ...
        private static AbstractContainer resolveContainer(ModelNode node) {
            if (node instanceof AreaNodeImpl a) {
                ...
                throw new IllegalArgumentException("node must be a live node obtained from "
                        + "Composer#getModelNode(AreaView) (or reached by navigating from it via "
    ...
    /**
     * Represents a potential drop position for a {@link TabView} or {@link TabDockView} during a drag-and-drop
     * operation.
     */
    private static final class DropPosition {
         ...
         */
        private Consumer<TabDockView<?>> transformation;
        ...
        public Consumer<TabDockView<?>> getTransformation() {
            return transformation;
        }

        public void setTransformation(Consumer<TabDockView<?>> transformation) {
    ...
    private static final class DropPositionResolver {
        ...
        DropPositionResolver(DockHostView<?> dockHost) {
            this.dockHost = dockHost;
        ...
        private void prepareForSameOrientationOnEdge(DropPosition dropPos, Side side) {
            ...
            Consumer<TabDockView<?>> transformation = null;
        ...
        private void prepareForSameOrientationOffEdge(DropPosition dropPos, Side side) {
            boolean isFirst = side == Side.TOP || side == Side.LEFT;
            var operation = resolveOperation();
            Consumer<TabDockView<?>> transformation = (newTabDock) -> {
        ...
        private void prepareForOppositeOrientationOffEdge(DropPosition dropPos, Side side) {
            ...
            var operation = resolveOperation();
            Orientation orientation = side.isVertical() ? Orientation.HORIZONTAL : Orientation.VERTICAL;
            Consumer<TabDockView<?>> transformation = (tabDock) -> {
    ...
    private static final class Transformer {
        ...
        Transformer(DockHostView<?> dockHost) {
            this.dockHost = dockHost;
            this.composer = dockHost.getComposer();
        }
         ...
         */
        private List<AbstractContainer> resolveParticipants(TabDockOperation operation, TabDockView<?> dock,
            ...
            if (chosen.isEmpty()) {
                throw new IllegalArgumentException("SpaceResolver for " + operation + " on "
                        + dock.getViewModel().getDescriptor().getFullName() + " returned an empty list");
            }
            ...
            for (var node : chosen) {
                ...
                if (!offered.contains(container)) {
                    throw new IllegalArgumentException("SpaceResolver for " + operation + " on "
                            + dock.getViewModel().getDescriptor().getFullName()
                            + " returned a node that was not offered: " + container.getChildFullName());
                }
                if (!seen.add(container)) {
                    throw new IllegalArgumentException("SpaceResolver for " + operation + " on "
                            + dock.getViewModel().getDescriptor().getFullName() + " returned a duplicate node: "
        ...
        private void addTabDock(Side side, ContainerPosition anchorInfo, ContainerPosition siblingInfo,
                ContainerPosition newInfo, TabDockView<?> newTabDock, TabDockOperation operation) {
            ...
            logger.debug("{} Added {} into {}; operation: {}", dockHost.getDescriptor().getLogPrefix(),
                    newTabDock.getViewModel().getDescriptor().getFullName(), splitPane.getFullName(), operation);
         ...
         */
        private void addTabDock(AbstractContainer anchorContainer, Side side, TabDockView<?> dock, double size,
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("{} Added {} at {} of {}; donors: {}, operation: {}",
                        ...
                        dock.getViewModel().getDescriptor().getFullName(), side, anchorContainer.getChildFullName(),
         ...
         */
        private void addTabDock(SplitPaneContainer parentContainer, TabDockView<?> dock, int index, Side side,
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("{} Added {} into {}; donors: {}, operation: {}", dockHost.getDescriptor().getLogPrefix(),
                        dock.getViewModel().getDescriptor().getFullName(),
                        dockContainer.getLogicalParent().getChildFullName(),
                        participants.stream().map(AbstractContainer::getChildFullName).toList(), operation);
                dockHost.printTreeDebugInfo();
            }
        }
         ...
         */
        private TabDockView<?> wrapAndAddTabDock(Orientation newOrientation, ContainerPosition anchorInfo,
                ContainerPosition newInfo, TabDockView<?> newTabDock, TabDockOperation operation) {
            newTabDock.getComposer().setDockHost(dockHost);
            ...
            logger.debug("{} Added {} to {}; wrap, sole donor: {}, operation: {}",
                    dockHost.getDescriptor().getLogPrefix(), newTabDock.getViewModel().getDescriptor().getFullName(),
         ...
         */
        private void removeTabDock(TabDockView<?> tabDock, TabDockOperation operation) {
        ...
        private void removeTabDock(TabDockContainer tabDockContainer, TabDockOperation operation) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("{} Removing {}; receivers: {}, operation: {}", dockHost.getDescriptor().getLogPrefix(),
                        operation, componentToRemove.getViewModel().getDescriptor().getFullName(),
                    participants.stream().map(AbstractContainer::getChildFullName).toList(), operation);
            }
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("{} Removed {} from {}; operation: {}", dockHost.getDescriptor().getLogPrefix(),
                        componentToRemove.getViewModel().getDescriptor().getFullName(),
                        parentContainer.getChildFullName(), operation);
            }
        ...
        private void moveTab(TabDockView<?> newTabDock) {
            ...
            TabPaneProSkin.TabHeaderArea tabHeaderArea = skin.getTabHeaderArea();
            TabView<?> tabView = (TabView<?>) ViewUtils.getView(dragTab);
            TabHostView<?> oldTabHost = (TabHostView<?>) tabView.getComposer().getParent();
            oldTabHost.getComposer().removeTab(tabView);
            newTabDock.getComposer().addTab(tabView);
     ...
     */
    private void minimizeTabDock(TabDockView<?> dock) {
        ...
        dock.getViewModel().minimizedPositionWrapper().set(pos);
        if (logger.isDebugEnabled()) {
            logger.debug("{} Minimized position for {}: {}; operation: {}", dockHost.getDescriptor().getLogPrefix(),
                    dock.getViewModel().getDescriptor().getFullName(), pos, operation);
        }
        var parentContainer = tabDockContainer.getLogicalParent();
        var splitPane = parentContainer.getSplitPane();
        var oldItems = List.copyOf(splitPane.getItems());
        ...
        private void restoreTabDock(TabDockContainer tabDockContainer) {
            ...
            dock.getViewModel().minimizedPositionWrapper().set(null);
            var parentContainer = tabDockContainer.getLogicalParent();
            var splitPane = parentContainer.getSplitPane();
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("{} Restored {} into {}; donors: {}, operation: {}",
                        dockHost.getDescriptor().getLogPrefix(), dock.getViewModel().getDescriptor().getFullName(),
     ...
     *
     * @see DockHostView.Composer#getModelNode(AreaView)
     */
    ...
    private static final Logger logger = LoggerFactory.getLogger(DockHostView.class);

    private static final Double DRAG_REGION_MAX_WIDTH = 200.0;

    public class Composer extends AbstractAreaView<VM>.Composer implements AreaView.Composer, DockHostComposer {

        private final DockHostView<VM> view = DockHostView.this;

        private PlaceholderView placeholder;

        private SplitPaneContainer rootContainer = null;

        private final ReadOnlyObjectWrapper<AreaView<?>> main = new ReadOnlyObjectWrapper<>();
         ...
         */
        private final ReadOnlyObjectWrapper<TabPopupView<?>> rightPopup = new ReadOnlyObjectWrapper<>();
         ...
         * Popups are DockHost children because they are added to DockHost center pane.
         */
        private final ReadOnlyObjectWrapper<TabPopupView<?>> bottomPopup = new ReadOnlyObjectWrapper<>();
         ...
         * Popups are DockHost children because they are added to DockHost center pane.
         */
        private final ReadOnlyObjectWrapper<TabPopupView<?>> leftPopup = new ReadOnlyObjectWrapper<>();
         ...
         * (see {@link #getModelNode(AreaView)}).
         *
         ...
         * <p>
         * Unlike the live nodes returned by {@link #getModelNode(AreaView)}, the returned tree is an independent
         ...
         */
        public AreaNode getModelNode(AreaView<?> area) {
         ...
         * to determine the side a {@link TabDockView} should be minimized to relative to it.
         *
         * @return the main area property
         */
        public final ReadOnlyObjectProperty<AreaView<?>> mainProperty() {
         ...
         */
        public final @Nullable AreaView<?> getMain() {
            return main.get();
        }

        @Override
        public @Nullable AreaPort getMainPort() {
            return getMain() == null ? null : getMain().getViewModel();
        }
         ...
         * <p>
         * {@code node} must be a live node — obtained from {@link #getModelNode(AreaView)}, or reached from it via
         ...
         * @throws IllegalArgumentException if {@code node} is not a live node obtained from this docking layout
         */
        public void addTabDock(TabDockView<?> tabDock, ModelNode node, Side side, double size) {
         ...
         */
        public void removeTabDock(TabDockView<?> dock) {
         ...
         */
        public void closeTabDock(TabDockView<?> dock) {
        ...
        public @Nullable SideBarPort getRightBarPort() {
            return getRightBar() == null ? null : getRightBar().getViewModel();
        }

        public final @Nullable SideBarView<?> getRightBar() {
            return rightBar.get();
        }
        ...
        public @Nullable SideBarPort getBottomBarPort() {
            return getBottomBar() == null ? null : getBottomBar().getViewModel();
        }

        public final @Nullable SideBarView<?> getBottomBar() {
        ...
        public @Nullable SideBarPort getLeftBarPort() {
            return getLeftBar() == null ? null : getLeftBar().getViewModel();
        }

        public final @Nullable SideBarView<?> getLeftBar() {
            return leftBar.get();
        }
        ...
        public @Nullable SideBarPort getBarPort(Side side) {
            var barView = view.getComposer().resolveBarWrapper(side).get();
            if (barView != null) {
                return barView.getViewModel();
            } else {
                return null;
            }
        }

        public final @Nullable SideBarView<?> getBar(Side side) {
            var wrapper = resolveBarWrapper(side);
            return wrapper.get();
        }

        public ReadOnlyObjectProperty<SideBarView<?>> resolveBarProperty(Side side) {
        ...
        public @Nullable TabPopupPort getRightPopupPort() {
            return getRightPopup() == null ? null : getRightPopup().getViewModel();
        }

        @Override
        public @Nullable TabPopupPort getBottomPopupPort() {
            return getBottomPopup() == null ? null : getBottomPopup().getViewModel();
        }

        @Override
        public @Nullable TabPopupPort getLeftPopupPort() {
            return getLeftPopup() == null ? null : getLeftPopup().getViewModel();
        }

        public final @Nullable TabPopupView<?> getRightPopup() {
            return rightPopup.get();
        }

        public final ReadOnlyObjectProperty<TabPopupView<?>> rightPopupProperty() {
            return rightPopup.getReadOnlyProperty();
        }

        public final @Nullable TabPopupView<?> getBottomPopup() {
            return bottomPopup.get();
        }

        public final ReadOnlyObjectProperty<TabPopupView<?>> bottomPopupProperty() {
            return bottomPopup.getReadOnlyProperty();
        }

        public final @Nullable TabPopupView<?> getLeftPopup() {
            return leftPopup.get();
        }

        public final ReadOnlyObjectProperty<TabPopupView<?>> leftPopupProperty() {
            return leftPopup.getReadOnlyProperty();
        }

        public final @Nullable TabPopupView<?> getPopup(Side side) {
            var wrapper = resolvePopupWrapper(side);
            return wrapper.get();
        }

        public ReadOnlyObjectProperty<TabPopupView<?>> resolvePopupProperty(Side side) {
            return resolvePopupWrapper(side).getReadOnlyProperty();
        }

        @Override
        public @Nullable TabPopupPort getPopupPort(Side side) {
        ...
        void addTabPopup(TabPopupView<?> popup) {
            ...
            var wrapper = view.getComposer().resolvePopupWrapper(popup.getViewModel().getSide());
            wrapper.set(popup);
        }
        ...
        void restoreTabDock(TabDockView<?> tabDock) {
            view.transformer.restoreTabDock(ContainerUtils.getContainer(tabDock));
        }

        void minimizeTabDock(TabDockView<?> tabDock) {
         ...
         */
        private void setMain(AreaView<?> value) {
            this.main.set(value);
        }
        ...
        private AbstractContainer build(ModelNode node) {
            if (node instanceof AreaNode areaNode) {
                ...
                updateDividerPositions(splitPane, groupNode.getChildren());
        ...
        private void updateDividerPositions(SplitPane splitPane, List<ModelNode> children) {
        ...
        private void setRightPopup(TabPopupView<?> value) {
        ...
        private void setBottomPopup(TabPopupView<?> value) {
        ...
        private void setLeftPopup(TabPopupView<?> value) {
            this.leftPopup.set(value);
        }

        private ReadOnlyObjectWrapper<TabPopupView<?>> resolvePopupWrapper(Side side) {
            return switch (side) {
        ...
        private ReadOnlyObjectWrapper<SideBarView<?>> resolveBarWrapper(Side side) {
            return switch (side) {
                case RIGHT -> rightBar;
    ...
    /**
     * Contains the {@link DockHostView#centerStackPane} and three {@link SideBarView}.
     */
    ...
    public DockHostView(VM viewModel) {
        super(viewModel);
    }
    ...
    protected Composer createComposer() {
        return new DockHostView.Composer();
    }
    ...
    /**
     * Creates the {@code DragAndDropContext} used by this {@link DockHostView}.
     */
    ...
    protected void build() {
        ...
        var css = DockHostView.class.getResource("dock-host.css").toExternalForm();
    ...
    protected void bind() {
        super.bind();
        getViewModel().centerWidthWrapper().bind(centerStackPane.widthProperty());
        getViewModel().centerHeightWrapper().bind(centerStackPane.heightProperty());
    }
    ...
    private AbstractAreaContainer<?> createContainer(AreaView<?> child) {
    ...
    private String getTreeDebugInfo() {
        ...
        ContainerUtils.traverse(getComposer().rootContainer, 0, (container, level) -> {
            ...
            if (container instanceof SplitPaneContainer spc) {
                ...
                builder.append(aac.getArea().getViewModel().getDescriptor().getState().name());
            }
            builder.append("]");
        });
