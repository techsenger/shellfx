...
class BodyRowView extends AbstractRowView<BodyRowViewModel> implements Cell<Integer, Node> {
    ...
    BodyRowView(BodyRowViewModel viewModel, HexAreaView<?> area) {
        super(viewModel, area);
    }
    ...
    public void updateItem(Integer offset) {
        var vm = getViewModel();
        var row = RowModel.create(offset, vm.getArea());
        vm.setModel(row);
        removeCaret();
        if (offset != null) {
            getNode().setVisible(true);
            var offsetStr = NumberBaseUtils.convert(row.getOffset(), vm.getArea().getToolBar().getOffsetNumberBase(),
                    vm.getArea().getOffsetLength());
            getInfoLabel().setText(offsetStr);
            for (var i = 0; i < vm.getArea().getToolBar().getRowByteCount(); i++) {
    ...
    public void rebuild() {
        ...
        var areaViewModel = viewModel.getArea();
...
//        }
    ...
    private void calculateCaretX(CaretPosition position) {
        var caretVM = getViewModel().getArea().getCaret();
