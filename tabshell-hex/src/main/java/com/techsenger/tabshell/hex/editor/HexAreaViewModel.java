 ...
 */
public class HexAreaViewModel<T extends HexAreaMediator> extends AbstractAreaViewModel<T> {
    ...
    public HexAreaViewModel(AppearanceSettings settings, HexDocument document) {
    ...
    public int calculateRowIndex(int offset) {
        int rowIndex = offset / getMediator().getToolBar().getRowByteCount();
        return rowIndex;
    }

    public int calculateByteIndex(int offset) {
        var byteIndex = offset % getMediator().getToolBar().getRowByteCount();
        return byteIndex;
    }
    ...
    protected int getRowByteCount(int rowIndex) {
        if (rowIndex + 1 == this.offsets.size()) {
            ...
            return getMediator().getToolBar().getRowByteCount();
        }
    }
    ...
    protected void initialize() {
        ...
        toolBar.rowByteCountProperty().addListener((ov, oldV, newV) -> updateLayout(true, null));
        toolBar.columnsEnabledProperty().addListener((ov, oldV, newV) -> updateLayout(false, null));
        toolBar.columnByteCountProperty().addListener((ov, oldV, newV) -> updateLayout(false, null));
        columnSeparator.addListener((ov, oldV, newV) -> updateLayout(false, null));
        toolBar.offsetNumberBaseProperty().addListener((ov, oldV, newV) -> {
            updateOffsetLength();
        ...
        selection.addListener((ov, oldV, newV) -> {
            if (newV == null) {
                ...
                logger.debug("{} Selection updated. Range: {}. Info : {}",
                        getMediator().getLogPrefix(), newV, selectionInfo);
            }
        });
        getMediator().getCaret().shapeProperty().addListener((ov, oldV, newV) -> adjustCaretOnShapeChange(oldV, newV));
    }
    ...
    int calculateRowIndex(RowData data) {
        int rowIndex = data.getOffset() / getMediator().getToolBar().getRowByteCount();
        return rowIndex;
    }
    ...
    RowData createRowData(Integer offset) {
        ...
        int realLength = Math.min(getMediator().getToolBar().getRowByteCount(), content.length - offset);
    ...
    void moveCaretUp() {
        var currentPos = getMediator().getCaret().getPosition();
    ...
    void moveCaretLeft() {
        ...
        if (atFirstByte && notAtFirstRow && canChangeRow) {
            //previous row
            moveCaretTo(currentPos.getPanel(), currentPos.getRowIndex() - 1,
                    getMediator().getToolBar().getRowByteCount() - 1, CaretByteLocation.SECOND);
        } else {
            //same row
    ...
    void moveCaretRight() {
        var caret = getMediator().getCaret();
        var currentPos = caret.getPosition();
        ...
        if (atLastByte && !atLastRow && canChangeRow) {
            ...
            if (currentPos.getPanel() == EditorPanel.HEX) {
                if (!(currentPos.getByteIndex() + 1 >= rowByteCount
                    ...
                    if (getMediator().getCaret().getShape() == CaretShape.BAR && atLastRow) {
                ...
                if (currentPos.getByteIndex() + 1 != rowByteCount) {
                    ...
                    if (caret.getShape() == CaretShape.BAR && atLastRow) {
                        byteLocation = CaretByteLocation.THIRD;
                    }
                }
                moveCaretTo(currentPos.getPanel(), currentPos.getRowIndex(), byteIndex, byteLocation);
    ...
    void moveCaretEnd() {
        ...
        var rowByteCount = getRowByteCount(currentPos.getRowIndex());
        var byteLocation = currentPos.getByteLocation();
        if (caret.getShape() == CaretShape.BAR) {
            byteLocation = CaretByteLocation.THIRD;
        } else {
            byteLocation = CaretByteLocation.SECOND;
        }
    ...
    private void updateLayout(boolean updateOffsets, CaretPosition pos) {
        ...
        if (updateOffsets) {
            ...
            if (pos == null) {
                var rowIndex = calculateRowIndex(caret.getOffset());
                var byteIndex = calculateByteIndex(caret.getOffset());
                var curPos = caret.getPosition();
    ...
    private void calculateFixedLayout() {
        ...
        var toolBar = getMediator().getToolBar();
        if (toolBar.areColumnsEnabled()) {
            this.columnCount.set(toolBar.getRowByteCount() / toolBar.getColumnByteCount());
        } else {
...
//    }
    ...
    private int calculateLastRowByteCount() {
        var toolBar = getMediator().getToolBar();
        var lastRowByteCount = this.document.getContent().length % toolBar.getRowByteCount();
        if (lastRowByteCount == 0 && this.document.getContent().length > 0) {
            lastRowByteCount = toolBar.getRowByteCount();
        }
