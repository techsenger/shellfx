 ...
 */
public abstract class AbstractDialogView<VM extends AbstractDialogViewModel<?>>
        extends AbstractWindowView<VM> implements DialogView<VM> {
    ...
    public AbstractDialogView(VM viewModel) {
        super(viewModel);
    }
    ...
    protected void addListeners() {
        ...
        viewModel.getLeftButtons().addListener((ListChangeListener<ResultButtonName>) change -> {
            removeButtons(leftBottomBox);
            addButtons(leftBottomBox, viewModel.getLeftButtons());
        });
        viewModel.getRightButtons().addListener((ListChangeListener<ResultButtonName>) change -> {
            removeButtons(rightBottomBox);
            addButtons(rightBottomBox, viewModel.getRightButtons());
        });
        viewModel.getDisabledByName().addListener((MapChangeListener<ResultButtonName, Boolean>) change -> {
            var button = this.buttonsByName.get(change.getKey());
            if (button != null && change.wasAdded()) {
                button.setDisable(change.getValueAdded());
            }
        });
        viewModel.getDefaultByName().addListener((MapChangeListener<ResultButtonName, Boolean>) change -> {
            var button = this.buttonsByName.get(change.getKey());
            if (button != null && change.wasAdded()) {
                button.setDefaultButton(change.getValueAdded());
            }
        });
    }
    ...
    /**
     * Registers a result button. After registration button can be modified only from the view model.
     *
     * @param buttons
     */
    protected void registerButtons(ResultButton... buttons) {
        for (var button: buttons) {
            ...
            button.setOnAction((e) -> {
                getViewModel().onResult(button.getName());
            });
            getViewModel().onButtonRegistered(button.getName(), button.isDefaultButton(), button.isDisable());
        }
    }
    ...
    protected void unregisterButtons(ResultButton... buttons) {
        for (var button : buttons) {
            ...
            getViewModel().onButtonUnregistered(button.getName());
        }
    }
    ...
    private void addButtons(HBox box, List<ResultButtonName> names) {
        for (var name : names) {
