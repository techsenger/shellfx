    ...
    default void closeSafely(int maxAttempts, Consumer<CloseRequestResult> resultConsumer) {
        class Requester {
            ...
            private void run() {
                ...
                SafeClosePort notReadyComponent = null;
                SafeClosePort prepRequiredComponent = null;
                var iterator = getComposer().breadthFirstPortIterator();
                while (iterator.hasNext()) {
                    var port = iterator.next();
                    if (port instanceof SafeClosePort closeable) {
                ...
                if (checkResult != CloseCheckResult.READY && logger().isDebugEnabled()) {
                    ...
                    var tree = getComposer().toPortTreeString((c, b) -> {
