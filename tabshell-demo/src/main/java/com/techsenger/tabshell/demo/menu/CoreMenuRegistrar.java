...
import com.techsenger.tabshell.demo.tab.DemoTabFxView;
import com.techsenger.tabshell.demo.tab.DemoTabPresenter;
 ...
 */
public class CoreMenuRegistrar extends AbstractControlRegistrar {

    public CoreMenuRegistrar(ControlRegistry registry) {
        super(registry);
    }
    ...
    protected void registerDemoMenu() {
        ControlFactory<NamedMenu> f = (v) -> {
            return new NamedMenu(CoreMenus.DEMO, "_Demo", 100);
        };
    ...
    protected void registerGroupOne() {
        ControlFactory<NamedMenuGroup> f = (v) -> {
            return new NamedMenuGroup(CoreMenus.ONE, 100);
        };
        addRegistration(getRegistry().registerMenuGroup(CoreComponents.SHELL, CoreMenus.DEMO, f));
    }

    protected void registerGroupTwo() {
        ControlFactory<NamedMenuGroup> f = (v) -> {
            return new NamedMenuGroup(CoreMenus.TWO, 200);
        };
        addRegistration(getRegistry().registerMenuGroup(CoreComponents.SHELL, CoreMenus.DEMO, f));
    }
    ...
    protected void registerNewItem() {
        ControlFactory<NamedMenuItem> f = (v) -> {
            var item = new NamedMenuItem(CoreMenus.NEW, false, true, false, "_New", 100);
            ...
            item.setOnAction((e) -> {
                ...
                ((TabHostFxView<?>) shellView.getWorkspace()).getComposer().addTab(tabView);
                // shellView.getComposer().addTab(tabView);
            });
            return item;

        };
        addRegistration(getRegistry().registerMenuItem(CoreComponents.SHELL, CoreMenus.ONE, f));
    }
    ...
    protected void registerExitItem() {
        ControlFactory<NamedMenuItem> f = (v) -> {
            var item = new NamedMenuItem(CoreMenus.EXIT, true, true, false, "E_xit", 100);
        ...
        addRegistration(getRegistry().registerMenuItem(CoreComponents.SHELL, CoreMenus.TWO, f));
    }
}
