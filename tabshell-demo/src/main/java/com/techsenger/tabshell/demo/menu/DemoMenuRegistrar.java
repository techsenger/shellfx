...
import com.techsenger.tabshell.demo.dock.DockHostTabFxView;
import com.techsenger.tabshell.demo.dock.DockHostTabPresenter;
...
public class DemoMenuRegistrar extends AbstractControlRegistrar  {
    ...
    public DemoMenuRegistrar(ControlRegistry registry, TabContainerFxView<?> tabContainer) {
        super(registry);
...
//
    ...
    protected void registerDockLayoutItem() {
        ControlFactory<NamedMenuItem> f = (v) -> {
            var item = new NamedMenuItem(DemoMenu.DOCK_LAYOUT, "Dock Host", 600);
            item.setOnAction((e) -> {
                var shell = (ShellFxView<?>) v;
                var dockTabView = new DockHostTabFxView(shell);
                var dockTabPresenter =
                        new DockHostTabPresenter(dockTabView, shell.getPresenter().getHistoryManager());
                dockTabPresenter.initialize();
                tabContainer.getComposer().addTab(dockTabView);
            });
            return item;
        };
        addRegistration(getRegistry().registerMenuItem(CoreComponents.SHELL, DemoMenu.DEFAULT, f));
    ...
    protected void registerWebBrowserItem() {
        ControlFactory<NamedMenuItem> f = (v) -> {
            ...
            item.setOnAction((e) -> {
                ...
                tabContainer.getComposer().addTab(browserView);
            });
            return item;
        };
        addRegistration(getRegistry().registerMenuItem(CoreComponents.SHELL, DemoMenu.DEFAULT, f));
    ...
    protected void registerPagedTabItem() {
        ControlFactory<NamedMenuItem> f = (v) -> {
            ...
            item.setOnAction((e) -> {
                ...
                tabContainer.getComposer().addTab(tabView);
