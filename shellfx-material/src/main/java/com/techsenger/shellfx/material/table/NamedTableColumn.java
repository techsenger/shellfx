...
/**
 * A {@link TableColumn} that carries a {@link TableColumnName} identity alongside JavaFX's own header text, so
 * {@link TableColumnManager}/{@link AbstractTableColumnManager} can track and rebuild it (position, width, sort)
 * by that stable identity rather than by header text, which is free-form and may not even be unique.
 *
 * @param <S> the row type of the {@code TableView} this column belongs to
 * @param <T> the cell value type this column displays
 * @author Pavel Castornii
 */
public class NamedTableColumn<S, T> extends TableColumn<S, T> implements Named {
