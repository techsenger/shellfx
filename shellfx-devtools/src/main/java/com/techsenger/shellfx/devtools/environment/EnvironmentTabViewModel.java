...
public class EnvironmentTabViewModel<C extends EnvironmentTabComposer> extends AbstractTabViewModel<C> {
    ...
    private final ObservableList<EnvironmentItem> items = FXCollections.unmodifiableObservableList(modifiableItems);

    private final DevToolsTabDockPort tabDock;

    private final Map<EnvironmentCategory, BooleanProperty> expandedByCategory;

    public EnvironmentTabViewModel(EnvironmentTabParams params) {
        super(params);
    ...
    public @Unmodifiable ObservableList<EnvironmentItem> getItems() {
        return items;
    }
