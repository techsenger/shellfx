 ...
 */
public class AlertDialogView<VM extends AlertDialogViewModel<?>> extends AbstractDialogView<VM> {
    ...
    public AlertDialogView(VM viewModel) {
        super(viewModel);
    }
    ...
    protected void build() {
        super.build();
        getContentBox().getStylesheets().add(AlertDialogView.class.getResource("alert.css").toExternalForm());
        messageIconViewBox.getStyleClass().add("icon-view-box");
        switch (getViewModel().getDialogType()) {
            case INFO:
    ...
    protected void bind() {
        ...
        messageIconViewBox.iconProperty().bind(viewModel.messageIconProperty());
    }

    protected IconViewBox getMessageIconViewBox() {
